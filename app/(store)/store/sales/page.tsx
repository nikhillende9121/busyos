...
// Trimmed copy of app/(dashboard)/sales/page.tsx for a warehouse-scoped
// Store Manager (see Docs/STORE_APP_GUIDE.md) — same schema/validation,
// same building blocks, only difference: no warehouse picker. The
// server already scopes/filters every one of these calls to the
// caller's own warehouse (shared/utils/assert-warehouse-access.ts); this
// page just doesn't show a control for a value that only ever has one
// answer.
export default function StoreSalesPage() {
  ...
  const onSubmit = async (values: FieldValues) => {
    try {
      const payload = { ...values, warehouseId: user!.warehouseId, couponCode: values.couponCode || undefined };
