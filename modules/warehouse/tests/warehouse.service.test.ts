...
vi.mock("@/shared/utils/plan-limits", () => ({
  getActivePlanLimits: vi.fn().mockResolvedValue({ maxWarehouses: null, maxUsers: null, maxRoles: null, maxWebhooks: null }),
}));
...
describe("warehouseService.create", () => {
  ...
  it("blocks creation once the plan's warehouse limit is reached", async () => {
    vi.mocked(getActivePlanLimits).mockResolvedValue({ maxWarehouses: 2, maxUsers: null, maxRoles: null, maxWebhooks: null });
