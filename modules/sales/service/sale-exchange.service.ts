...
import type { CreateSaleExchangeDto } from "../dto/sale-exchange.dto";
import type { SaleExchangeView } from "../types/sale-exchange.types";
...
export const saleExchangeService = {
  ...
  async create(dto: CreateSaleExchangeDto): Promise<SaleExchangeView> {
    ...
    const returnLines = dto.returnItems.map((returnItem) => {
    ...
    const newItemsTotal = lineTaxResults
      ...
      .add(
        resolvedCharges.reduce(
          (sum, charge) => sum.add(charge.amount).add(charge.taxAmount),
